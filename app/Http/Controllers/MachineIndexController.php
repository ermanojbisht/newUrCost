...
class MachineIndexController extends Controller
{
    ...
    {
        if ($resource->resource_group_id != 2) {
            abort(403, 'Machine Index is only applicable for Machine resources.');
        }

        if ($request->ajax()) {
            $query = MachineIndex::with(['rateCard', 'createdBy'])
        ...
        $totalFallback = 0;
        if ($resource->id != 1) {
             $query = \App\Models\MachineIndex::with(['rateCard', 'createdBy'])
                ->where('resource_id', 1)
                ->where('is_canceled', 0);
        ...
        return view('resources.machine_indices.index', compact('resource', 'rateCards', 'fallbackIndices', 'totalFallback'));
    }
    ...
    {
        $index = MachineIndex::findOrFail($id);
    ...
    public function lock(Request $request, $id)
    {
        ...
        $currentRate = MachineIndex::where('resource_id', $index->resource_id)
    ...
    {
        if ($request->ajax()) {
            $query = MachineIndex::with(['resource', 'rateCard', 'createdBy'])
                ->where('is_canceled', 0);
        ...
        $rateCards = RateCard::all();
        $resources = Resource::where('resource_group_id', 2)->get(); // Machine resources

        return view('resources.machine_indices.all', compact('rateCards', 'resources'));
    }
}
