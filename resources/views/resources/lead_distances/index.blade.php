...
@section('title', 'Manage Lead Distances - ' . $resource->name)
...
<div class="container mx-auto px-4 py-6">
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white flex items-center">
                {!! config('icons.truck') !!}
                <span class="ml-2">Manage Lead Distances: {{ $resource->name }}</span>
            </h1>
        ...
        <div class="flex space-x-2">
            <button onclick="openModal()" class="btn-primary flex items-center">
                {!! config('icons.add') !!}
                <span class="ml-2">Add Lead Distance</span>
            </button>
    ...
    @if($resource->id != 1 && $fallbackIndices->count() > 0)
    <div class="mt-8">
        <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Global / Fallback Indices (Resource ID 1)</h2>
        <div class="bg-gray-50 dark:bg-gray-700 rounded-lg shadow overflow-hidden">
            <div class="p-4">
                    ...
                    <tbody>
                        @foreach($fallbackIndices as $index)
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                <td class="px-6 py-4">{{ $index->rateCard->name ?? '-' }}</td>
                                <td class="px-6 py-4">{{ $index->station->name ?? '-' }}</td>
                                <td class="px-6 py-4">{{ $index->distance }} km</td>
                                <td class="px-6 py-4">
                                    @if($index->type == 1) Mechanical
                                    @elseif($index->type == 2) Manual
                                    @elseif($index->type == 3) Mule
                                    @else {{ $index->type }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">{{ $index->valid_from ? $index->valid_from->format('d-M-Y') : '-' }}</td>
                                <td class="px-6 py-4">
                                    @if($index->is_locked == 0) <span class="text-blue-600">Experimental</span>
                                    @elseif($index->is_locked == 1) <span class="text-green-600">Current</span>
                                    @elseif($index->is_locked == 2) <span class="text-gray-600">Old</span>
                                    @endif
                                </td>
                            </tr>
                        @endforeach
                ...
                <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">
                    * These indices apply if no specific index is defined for this resource.
                </p>
            </div>
        </div>
    </div>
        ...
        <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full relative z-[10000]">
            <form id="indexForm">
                ...
                <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-white" id="modal-title">Add Lead Distance</h3>
                    <div class="mt-4 space-y-4">
                        ...
                        <div>
                            <label for="station_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Station (Optional)</label>
                            <select id="station_id" name="station_id" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white dark:bg-gray-700 dark:border-gray-600 dark:text-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                        ...
                        <div>
                            <label for="distance" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Distance (km)</label>
                            <input type="number" step="0.01" id="distance" name="distance" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white dark:bg-gray-700 dark:border-gray-600 dark:text-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                        </div>
                        <div>
                            <label for="type" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Type</label>
                            <select id="type" name="type" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white dark:bg-gray-700 dark:border-gray-600 dark:text-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                                <option value="1">Mechanical</option>
                                <option value="2">Manual</option>
                                <option value="3">Mule</option>
                            </select>
                        </div>
                        <div>
                ...
                <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-white" id="lock-modal-title">Lock Record</h3>
                            <div class="mt-2">
                                <p class="text-sm text-gray-500 dark:text-gray-400">
                                    Are you sure you want to lock this record? This will promote it to "Current" and archive the previous current record. This action cannot be undone.
                                </p>
                                <div class="mt-4">
...
<script>
    function openModal() {
        $('#indexForm')[0].reset();
        $('#index_id').val('');
        $('#modal-title').text('Add Lead Distance');
    ...
    $(document).ready(function() {
        var table = $('#indices-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('resources.lead-distances.index', $resource->id) }}",
            columns: [
                {data: 'id', name: 'id'},
                ...
                {data: 'type', name: 'type', render: function(data) {
                    if (data == 1) return 'Mechanical';
                    if (data == 2) return 'Manual';
                    if (data == 3) return 'Mule';
                    return data;
                }},
        ...
        $('#indexForm').on('submit', function(e) {
            ...
            if (id) {
                ...
                url = "{{ route('resources.lead-distances.store', $resource->id) }}";
                method = 'POST';
            }
        ...
        $('#lockForm').on('submit', function(e) {
            ...
            var url = "/lead-distances/" + id + "/lock";
        ...
        $('#indices-table').on('click', '.edit-btn', function() {
            var id = $(this).data('id');
            $.get("/lead-distances/" + id, function(data) {
                ...
                $('#modal-title').text('Edit Lead Distance');
                $('#indexModal').removeClass('hidden');
            });
        });
    });
