                ...
                <div>
                    ...
                    <select id="rate_card_id" x-model="selectedRateCard" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                        ...
                        @foreach($rateCards as $rateCard)
                            <option value="{{ $rateCard->id }}">{{ $rateCard->reference_desc }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <template x-for="file in files" :key="file.id">
                        <tr :class="{'bg-green-50 dark:bg-green-900': file.id === newFileId}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                ...
                                <span x-text="file.rate_card ? file.rate_card.reference_desc : 'N/A'"></span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full" 
                                      :class="file.status === 'Generated' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'"
    ...
    function sorExport() {
        return {
            ...
            generateReport() {
                ...
                const url = `{{ url('/export/sor') }}/${this.selectedSor}/rate-card/${this.selectedRateCard}/format/${this.selectedFormat}`;
