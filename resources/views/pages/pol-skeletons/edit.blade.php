...
@section('title', __('Edit POL Skeleton'))
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white flex items-center">
                ...
                <span>{{ __('Edit POL Skeleton') }}</span>
            </h1>
            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                {{ __('Update the details of the POL skeleton.') }}
            </p>
        </div>
    </div>
@endsection
...
<div class="container mx-auto">
    <div class="card">
        <form action="{{ route('pol-skeletons.update', $polSkeleton) }}" method="POST">
            @csrf
            @method('PUT')
            @include('pages.pol-skeletons._form')
