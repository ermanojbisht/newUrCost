...
@section('title', __('Create Truck Speed'))
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white flex items-center">
                ...
                <span>{{ __('Create Truck Speed') }}</span>
            </h1>
            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                {{ __('Add a new truck speed to the system.') }}
            </p>
        </div>
    </div>
@endsection
...
<div class="container mx-auto">
    <div class="card">
        <form action="{{ route('truck-speeds.store') }}" method="POST">
            @csrf
            @include('pages.truck-speeds._form', ['truckSpeed' => new \App\Models\TruckSpeed()])
            <div class="flex justify-end space-x-2 mt-6">
                <a href="{{ route('truck-speeds.index') }}" class="btn-secondary">{{ __('Cancel') }}</a>
