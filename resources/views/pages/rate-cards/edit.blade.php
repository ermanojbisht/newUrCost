...
@section('title', __('Edit Rate Card'))
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white flex items-center">
                ...
                <span>{{ __('Edit Rate Card') }}</span>
            </h1>
            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                {{ __('Update the details of the rate card.') }}
            </p>
        </div>
    </div>
@endsection
...
@section('content')
    <div class="card">
        <form action="{{ route('rate-cards.update', $rateCard) }}" method="POST">
            @csrf
            @method('PUT')
            @include('pages.rate-cards._form')
